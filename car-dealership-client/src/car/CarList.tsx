  ...
  return (
    <IonPage>
      ...
      <IonContent>
        ...
        {cars && (
          <IonList>
            {cars.map(({ id, model,sellDate,price,isElectric }) =>
              <Car key={id} id={id} model={model} price={price} isElectric={isElectric} sellDate={sellDate} onEdit={id => history.push(`/car/${id}`)} />)}
          </IonList>
        )}
        {fetchingError && (
